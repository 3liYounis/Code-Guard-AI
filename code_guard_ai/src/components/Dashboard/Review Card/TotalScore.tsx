...
const getStatus = (
  value: number
): ConditionalValue<"success" | "warning" | "error"> => {
...
const statusMap: { [key: string]: StatusMap } = {
  success: {
    icon: FaCheck,
    color: "green.600",
    description: "Looks like your code meets high standards!",
  },
  warning: {
    icon: TiWarning,
    color: "yellow.600",
  ...
  error: {
    ...
    color: "red.600",
    description: "Looks like your code has some critical issues!",
  },
  other: { icon: FaQuestion, color: "white.600", description: "" },
};
const TotalScore = ({ totalScore }: Props) => {
  const status = getStatus(totalScore);
  const description = (statusMap[status?.toString()] ?? statusMap["other"])
    .description;
  ...
  return (
    <Alert.Root status={status} width={370} color="ActiveBorder">
      ...
      <Alert.Content>
        <Alert.Title>Overall Score: {totalScore.toFixed(2)}</Alert.Title>
